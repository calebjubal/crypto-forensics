...
function denyNetwork() {
  const denied = () => {
    throw new Error(
      "Network access is disabled: this is an offline application.",
    );
  };
  ...
  tls.connect = tls.createServer = denied;
  require("node:dgram").createSocket = denied;
  const dns = require("node:dns");
  for (const key of Object.keys(dns))
    if (/^(resolve|lookup|reverse)/.test(key)) dns[key] = denied;
  for (const key of Object.keys(dns.promises))
    if (/^(resolve|lookup|reverse)/.test(key)) dns.promises[key] = denied;
