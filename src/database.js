...
function audit(db, action, details = {}, context = {}) {
  ...
  db.prepare("INSERT INTO audit(timestamp,action,details) VALUES(?,?,?)").run(
...
function validateCredentials(input, creating = false) {
  ...
  if (creating && (password.length < PASSWORD_MINIMUM || password.length > 256))
...
function createInitialUser(db, input) {
  if (authenticationStatus(db).configured)
    throw new Error("A local account is already configured.");
  ...
  try {
    db.prepare(
      "INSERT INTO users(username,salt,password_hash,created,last_login) VALUES(?,?,?,?,?)",
    ).run(
      username,
      salt.toString("hex"),
...
async function importFile(
  ...
  const insertObs = db.prepare(
    "INSERT OR IGNORE INTO observations(txid,timestamp,ts_ms,src_ip,dst_ip,src_port,dst_port,geo_country,asn,fingerprint) VALUES(?,?,?,?,?,?,?,?,?,?)",
  );
  ...
  try {
    db.prepare(
      "INSERT INTO imports(id,name,format,created) VALUES(?,?,?,?)",
    ).run(stats.id, stats.name, stats.format, stats.created);
    for await (const row of records(file, stats, cancelled)) {
      if (cancelled())
        throw new Error("Import cancelled; this file was rolled back.");
      stats.rows++;
      let record;
      try {
        record = normalize(row);
        const previous = findTx.get(record.txid);
        if (previous && previous.data_hash !== record.data_hash)
          throw new Error(
            "Conflicting blockchain data for an existing TXID. Existing evidence was preserved.",
          );
      } catch (error) {
        reject.run(stats.id, stats.rows, error.message);
        stats.rejected++;
        continue;
      }
      const r = record;
      if (!findTx.get(r.txid)) {
        insertTx.run(
          r.txid,
          JSON.stringify(r.input_addresses),
          JSON.stringify(r.output_addresses),
          JSON.stringify(r.input_amounts),
          JSON.stringify(r.output_amounts),
          r.input_sat,
          r.output_sat,
          r.fee_sat,
          r.coinjoin,
        ...
        for (const addr of r.input_addresses)
          insertAddress.run(r.txid, addr, "input");
        for (const addr of r.output_addresses)
          insertAddress.run(r.txid, addr, "output");
      ...
      if (stats.rows % 500 === 0)
        onProgress({
          ...
          percent: Math.min(
            99,
            Math.round((stats.bytes / Math.max(size, 1)) * 100),
          ),
        });
    }
    ...
    if (cancelled())
      throw new Error("Import cancelled; this file was rolled back.");
    db.prepare(
      "UPDATE imports SET sha256=?,bytes=?,rows=?,accepted=?,duplicates=?,rejected=?,new_transactions=? WHERE id=?",
    ).run(
      stats.sha256,
      stats.bytes,
      stats.rows,
    ...
    audit(
      db,
      "import.rolled_back",
      { name: stats.name, reason: error.message },
      context,
    );
    throw error;
  }
...
function deleteImport(db, id, context = {}) {
  ...
  try {
    db.exec(
      "DROP TABLE IF EXISTS temp.removed_observations; DROP TABLE IF EXISTS temp.orphan_transactions;",
    );
    db.prepare(
      "CREATE TEMP TABLE removed_observations AS SELECT observation_id AS id FROM provenance WHERE import_id=?",
    ).run(id);
    const linked = db
      .prepare("SELECT count(*) AS count FROM removed_observations")
      .get().count;
    db.prepare("DELETE FROM provenance WHERE import_id=?").run(id);
    const removedObservations = db
      .prepare(
        "DELETE FROM observations WHERE id IN (SELECT id FROM removed_observations) AND NOT EXISTS (SELECT 1 FROM provenance WHERE provenance.observation_id=observations.id)",
      )
      .run().changes;
    db.exec(
      "CREATE TEMP TABLE orphan_transactions AS SELECT txid FROM transactions WHERE NOT EXISTS (SELECT 1 FROM observations WHERE observations.txid=transactions.txid);",
    );
...
function summary(db) {
  ...
  const revision = Number(
    scalar("SELECT value FROM metadata WHERE key='revision'"),
  );
  return {
    transactions: scalar("SELECT count(*) FROM transactions"),
    observations: scalar("SELECT count(*) FROM observations"),
    addresses: scalar("SELECT count(DISTINCT address) FROM addresses"),
    clusters: scalar("SELECT count(*) FROM clusters WHERE size>1"),
    leads: scalar("SELECT count(*) FROM lead_scores WHERE score>=25"),
    urgent: scalar(
    ...
    priorities: db
      .prepare(
        "SELECT priority,count(*) AS count FROM lead_scores WHERE score>=25 GROUP BY priority",
      )
      .all(),
    countries: db
      .prepare(
        "SELECT coalesce(geo_country,'—') AS country,count(*) AS count FROM observations GROUP BY geo_country ORDER BY count DESC LIMIT 7",
      )
      .all(),
    timeline: db
      .prepare(
        "SELECT substr(timestamp,1,10) AS day,count(*) AS count,count(DISTINCT txid) AS tx_count FROM observations GROUP BY day ORDER BY day DESC LIMIT 30",
      )
      .all()
      .reverse(),
    run,
...
function page(db, type, options = {}) {
  ...
  if (type === "transactions" || type === "leads") {
    ...
    if (search) {
      where.push(
        "(t.txid LIKE ? OR EXISTS(SELECT 1 FROM addresses a WHERE a.txid=t.txid AND a.address LIKE ?) OR EXISTS(SELECT 1 FROM observations o WHERE o.txid=t.txid AND (o.src_ip LIKE ? OR o.dst_ip LIKE ?)))",
      );
      for (let i = 0; i < 4; i++) params.push(`%${search}%`);
    }
    if (options.priority && options.priority !== "All priorities") {
      where.push("l.priority=?");
      params.push(String(options.priority));
    }
    ...
    const rows = db
      ...
      ${select}${filters} ORDER BY ${order} LIMIT ? OFFSET ?`,
      )
...
function detail(db, txid) {
  ...
  const observations = db
    .prepare(
      "SELECT * FROM observations WHERE txid=? ORDER BY ts_ms,id LIMIT 250",
    )
    .all(txid);
  const total = db
    .prepare("SELECT count(*) AS n FROM observations WHERE txid=?")
    .get(txid).n;
  const sources = db
    .prepare(
      `SELECT i.id,i.name,i.sha256,count(*) AS row_count FROM provenance p JOIN observations o ON o.id=p.observation_id JOIN imports i ON i.id=p.import_id WHERE o.txid=? GROUP BY i.id`,
    )
    .all(txid);
  const clusters = db
    .prepare(
      "SELECT DISTINCT c.* FROM clusters c JOIN cluster_members m ON m.cluster_id=c.id JOIN addresses a ON a.address=m.address WHERE a.txid=? AND c.size>1",
    )
    .all(txid);
  const review = db
...
function review(db, input, context = {}) {
  if (!["New", "In review", "Escalated", "Dismissed"].includes(input.status))
    throw new Error("Invalid review status.");
  if (typeof input.notes !== "string" || input.notes.length > 10000)
    throw new Error("Notes must contain at most 10,000 characters.");
  if (!db.prepare("SELECT 1 FROM transactions WHERE txid=?").get(input.txid))
    throw new Error("Transaction not found.");
  db.exec("BEGIN");
  try {
    db.prepare(
      "INSERT INTO lead_reviews VALUES(?,?,?,?) ON CONFLICT(txid) DO UPDATE SET status=excluded.status,notes=excluded.notes,updated=excluded.updated",
    ).run(input.txid, input.status, input.notes, new Date().toISOString());
    audit(
      db,
      "lead.reviewed",
      { txid: input.txid, status: input.status },
      context,
    );
    db.exec("COMMIT");
    return { saved: true };
