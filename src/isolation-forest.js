...
function pathLength(row, node, depth = 0) {
  ...
  return pathLength(
    row,
    row[node.feature] < node.split ? node.left : node.right,
    depth + 1,
  );
}
function train(rows, { trees = 64, sampleSize = 256, seed = 73129 } = {}) {
  ...
  const random = seededRandom(seed),
    n = Math.min(sampleSize, rows.length),
    forest = [];
  for (let i = 0; i < trees; i++) {
    ...
    forest.push(
      build(
        indices.slice(0, n).map((j) => rows[j]),
        0,
...
function score(model, row) {
  ...
  const average =
    model.forest.reduce((sum, tree) => sum + pathLength(row, tree), 0) /
    model.trees;
