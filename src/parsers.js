...
async function* records(file, stats, cancelled = () => false) {
  ...
  if (!["csv", "json", "xml"].includes(format))
    throw new Error("Supported file types: CSV, JSON, XML.");
  ...
  const meter = new Transform({
    transform(chunk, encoding, callback) {
      if (cancelled())
        return callback(
          new Error(
            "Operation cancelled. No rows from this file were committed.",
          ),
        );
      digest.update(chunk);
      stats.bytes = (stats.bytes || 0) + chunk.length;
      callback(null, chunk);
    },
  });
  try {
    if (format === "xml") {
      ...
      let stack = [],
        ...
        size = 0;
      xml.on("doctype", () => {
        throw new Error("XML DTDs and external entities are forbidden.");
      });
      xml.on("opentag", (tag) => {
        stack.push({ name: tag.name, text: "", children: [] });
        if (stack.length > 5)
          throw new Error("XML nesting exceeds the supported schema.");
        if (stack.length === 1 && tag.name !== "records")
          throw new Error("XML root must be <records>.");
        if (stack.length === 2) {
          if (tag.name !== "record")
            throw new Error("XML rows must use <record>.");
          current = {};
          size = 0;
        }
      });
      ...
      xml.on("cdata", (value) => {
        throw new Error("CDATA is not supported; use escaped XML text.");
      });
      xml.on("closetag", () => {
        const node = stack.pop();
        if (stack.length === 3) stack[2].children.push(node.text.trim());
        if (stack.length === 2) {
          if (Object.hasOwn(current, node.name))
            throw new Error(`Duplicate XML field: ${node.name}`);
          current[node.name] = ARRAY_FIELDS.includes(node.name)
        ...
        columns(headers) {
          ...
          for (const field of REQUIRED)
            if (!headers.includes(field))
              throw new Error(`CSV header missing ${field}.`);
          return headers;
        },
        skip_empty_lines: true,
        max_record_size: MAX_RECORD,
      });
      ...
      const guard = new Transform({
        ...
        transform(token, encoding, callback) {
          if (token.name === "startObject" || token.name === "startArray")
            depth++;
          if (depth > 32)
            return callback(new Error("JSON nesting exceeds 32 levels."));
          if (token.name === "stringChunk" || token.name === "numberChunk")
            size += token.value.length;
          if (size > MAX_RECORD)
            return callback(new Error("JSON record exceeds 2 MiB."));
