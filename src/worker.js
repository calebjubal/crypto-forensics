...
const { openDatabase, importFile, deleteImport, summary, page, detail, clusterDetail, review, audit,
  authenticationStatus, createInitialUser, authenticate } = require('./database');
...
parentPort.on('message', async ({ id, action, payload = {}, context = {} }) => {
  ...
  try {
    let result;
    switch (action) {
      case 'auth.status': result = authenticationStatus(db); break;
      case 'auth.setup': result = createInitialUser(db,payload); break;
      case 'auth.login': {
        if (Date.now() < lockedUntil) throw new Error(`Too many failed attempts. Try again in ${Math.ceil((lockedUntil-Date.now())/1000)} seconds.`);
        try { result = authenticate(db,payload); loginFailures = 0; lockedUntil = 0; }
        catch (error) { if (++loginFailures >= 5) lockedUntil = Date.now() + 30000; throw error; }
        break;
      }
      ...
      case 'review': result = review(db,payload,context); break;
      case 'import': {
        result = [];
        for (const file of payload.files) { if (cancelled()) break; result.push(await importFile(db,file,progress,cancelled,context)); }
        break;
      }
      case 'delete-import': result = deleteImport(db,payload.id,context); break;
      case 'analyze': result = analyze(db,progress,cancelled,context); break;
      ...
      case 'export': result = await exportReport(db,payload.file,payload.format,cancelled,context); break;
      case 'audit': audit(db,payload.action,payload.details,context); result = true; break;
