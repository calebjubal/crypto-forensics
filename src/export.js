...
async function exportReport(
  ...
  try {
    ...
    const metadata = {
      ...
      schemaVersion: 1,
      warning:
        "Investigative hypotheses only. Scores are not probabilities of wrongdoing. IP observations do not establish wallet ownership. Source geo/ASN is supplied metadata, not verified geolocation.",
    ...
    for (const row of db
      .prepare(
        `SELECT l.*,coalesce(r.status,'New') AS status,coalesce(r.notes,'') AS notes FROM lead_scores l LEFT JOIN lead_reviews r ON r.txid=l.txid WHERE l.score>=25 ORDER BY score DESC,txid`,
      )
      .iterate()) {
    ...
    if (format === "json")
      await write(
        '],"audit":' +
          JSON.stringify(db.prepare("SELECT * FROM audit ORDER BY id").all()) +
          "}",
      );
    stream.end();
